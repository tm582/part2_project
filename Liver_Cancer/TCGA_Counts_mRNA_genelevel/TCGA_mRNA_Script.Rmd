...
##Edge R Analysis
#Set-Up DGE Object and initial organisation
```{r}
...
quartz()
plot(pca$x, pch=c(21,24)[batch], bg=cond_colours[conds], cex=1)
...
contr.matrix=makeContrasts(conds_originPrimary_Tumor-conds_originNormal_TCGA, conds_originPrimary_Tumor-conds_originNormal_GTEX, conds_originRecurrent_Tumor - conds_originNormal_TCGA, conds_originRecurrent_Tumor-conds_originNormal_GTEX, levels = colnames(design))
...
pca=prcomp(t(v$E))
quartz()
plot(pca$x, pch=c(21,24)[batch], bg=cond_colours[conds_origin], cex=1, title(main = 'PCA Plot'))
text(pca$x,as.character(coldata$Batch),cex=0.3,pos=1)
text(pca$x,as.character(colnames(v$E)),cex=0.3,pos = 3)
...
#MDS Plot
quartz()
plotMDS(efit, pch = 19, col= cond_colours[conds_origin], main='Multi Dimensional Scaling (MDS) Plot')
legend('bottomright', legend=levels(conds), col=cond_colours[unique(conds)], pch = 20)
...
plot(topTable(efit,coef=1,number=1000000000000000)$logFC,topTable(efit,coef=2,number=1000000000000000)$logFC, title(main='Butterfly Plot for Primary Tumour vs. TCGA & GTEX Normals'))
abline(h=0)
abline(v=0)
abline(a=0,b=1, lty=1, col='red')

plot(topTable(efit,coef=3,number=1000000000000000)$logFC,topTable(efit,coef=4,number=1000000000000000)$logFC, title(main='Butterfly Plot for Primary Tumour vs. TCGA & GTEX Normals'))
abline(h=0)
abline(v=0)
abline(a=0,b=1, lty=1, col='red')
```
...
dds=DESeqDataSetFromMatrix(mergedcounts, colData=coldata, design = ~Sample_Type + Batch)
...
quartz()
par(mfrow=c(1,2))
plot(log2(counts(dds, normalized=TRUE)+ 1), pch=16, cex=0.3, main = 'Log2 Normalised Counts')
...
#PCA Plot
pca=prcomp(t(assay(rlt)))
quartz()
plot(pca$x, pch=c(21,24)[batch], bg=cond_colours[conds], main='PCA Plot: rlog values')
  text(pca$x,as.character(coldata$Batch),cex=0.3,pos=1)
  text(pca$x,as.character(coldata$Sample_Type),cex=0.3,pos = 3)
...
topVarGenes=head(order(rowVars(assay(rlt)),decreasing=T), 50)
mat <- assay(rlt)[ topVarGenes, ]
mat <- mat - rowMeans(mat)

df <- as.data.frame(colData(rlt)[,c("Batch","Sample_Type")])

heatmap.2(mat, trace='none', cexRow = 0.5, cexCol = 0.5, col=hmcol)
heatmap.2(assay(rlt)[rownames(res[1:50,]),],col=hmcol,trace="none",cexRow = 0.5,cexCol = 0.5)
...
stripchart(svseq$sv[,1] ~ dds$Batch,vertical=TRUE,main="SV1")
abline(h=0)

stripchart(svseq$sv[,2] ~ dds$Batch,vertical=TRUE,main="SV2")
abline(h=0)
...
```{r}
mergenormcounts <- counts(ddssva, normalized=TRUE)
mergerawcounts=counts(ddssva,normalized=FALSE)
mergelog2counts=log2(mergenormcounts+1)
...
barplot(colSums(counts(ddssva, normalized=TRUE)), col=cond_colours[conds], las=2,cex.names=0.5,main='Post Normalised Counts')
legend("topright",levels(conds),cex=0.5,fill=cond_colours[levels(conds)])
...
```{r}
set.seed(72)
tsne = Rtsne(t(assay(ddssva)), perplexity = 3)
tsne.df=data.frame(tsne.1=tsne$Y[,1], tsne.2=tsne$Y[,2], Type=as.factor(ddssva$Sample_Type), Batch=as.factor(ddssva$Batch))
...
zz=counts(ddssva,normalized=T)[rownames(res[1:50,]),]
quartz()
heatmap.2(log2(zz+1),col=hmcol,trace="none")
```
