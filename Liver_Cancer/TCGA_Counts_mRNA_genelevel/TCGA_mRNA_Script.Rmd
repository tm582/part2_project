...
#P threshold is set to 0.01.
#log fold change threshold set to 1.5
#Highlighted hits limited to 250.
...
median.recurrent=apply(vstMat[,372:374],1,median)
median.normal=apply(vstMat[,375:424],1,median)
...
for (i in 2:(length(condition_list))){
  ...
  quartz()
  heatmap.2(mergevstMat[hits,],trace='none',col=hmcol,Colv=FALSE,dendrogram='col',main=paste("Heatmap Sig. Hits for", compare),labCol = conds, cexCol = 0.5) 
  } else {
    print(paste("No Results for ",compare))
  }
}
...
median.recurrent=apply(mergevstMat[,547:549],1,median)
median.normal=apply(mergevstMat[,c(1:175, 550:599)], 1, median)
...
hit_table2=topTable(efit,coef=2,number=10000000000000)
hit_table1=hit_table1[order(abs(hit_table1$logFC),decreasing = T),]
hit_table2=hit_table2[order(abs(hit_table2$logFC),decreasing = T),]

hitlist1=rownames(hit_table1[((abs(hit_table1$logFC) >= lfc_threshold) & (hit_table1$adj.P.Val<=p_threshold)),])
hitlist2=rownames(hit_table2[((abs(hit_table2$logFC) >= lfc_threshold) & (hit_table2$adj.P.Val<=p_threshold)),])
...
quartz()
plot(hit_table2$logFC,-log10(hit_table2$adj.P.Val),pch=19,cex=0.4,col="darkblue", main='Volcano Plot - Normal vs Recurrent Tumour (Voom)')
points(hit_table2[hitlist2,"logFC"],-log10(hit_table2[hitlist2,"adj.P.Val"]),cex=0.4,col="cyan")
text(hit_table2[1:100,]$logFC,-log10(hit_table2[1:100,]$adj.P.Val), labels= gene_names[hitlist2[1:100],]$GeneName, pos=3, cex=0.4)
  abline(h=-log10(p_threshold), lty=3, col='red')
  abline(v=-lfc_threshold, lty=3, col='red')
  abline(v=lfc_threshold, lty=3, col='red')
...
heatmap.2(lcpm[hitlist1[1:100],], trace = 'none', Colv=FALSE, col=hmcol, main = 'Heatmap of lcpm for Primary Tumour')
heatmap.2(lcpm[hitlist2[1:100],], trace = 'none', Colv=FALSE, col=hmcol, main = 'Heatmap of lcpm for Recurrent Tumour')
...
qqnorm(log2(dge$counts+1)[,598], main = 'Normal Q-Q Plot (TCGA Sample 1)')
qqnorm(log2(dge$counts+1)[,599], main = 'Normal Q-Q Plot (TCGA Sample 2)')
...
VOOMrechits=as.data.frame(gene_names[hitlist2[1:250],]$GeneName)
...
conds_origin[550:599]=rep("Normal_TCGA",58)
...
#This produces n.sv of 596  
#Error occured, run svaseq with decreasing n.sv value
svseq=svaseq(dat, mod, mod0, n.sv=594)
...
quartz()
  heatmap.2(svavstMat[hits,],trace='none',col=hmcol,Colv=FALSE,dendrogram='col',main=paste("Heatmap Sig. Hits for", compare),labCol = conds, cexCol = 0.5) 
...
SVArechits=as.data.frame(gene_names[hits,]$GeneName)
...
venn.diagram(x=c(SVAprimhits, SVArechits), filename='~/Desktop/part2_project/Liver_Cancer/plots/testvenn', na='remove', category.names = c('SVA Primary Hits', 'SVA Recurrent Hits', cat.cex=0.5))
