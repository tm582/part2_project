...
for (i in 1:(length(condition_list)-1)){
  ...
  text(res[hits,]$log2FoldChange,-log10(res[hits,]$padj), labels = 'hit', pos=3, cex = 0.4)
...
text(median.normal[hits],median.primary[hits],cex=0.4,pos=4,labels = 'hit')
...
text(median.normal[hits],median.recurrent[hits],cex=0.4,pos=4,labels = 'hit')
...
medians=data.frame(Primary_Tumor=median.primary, Recurrent_Tumor=median.recurrent, Solid_Tissue_Normal=median.normal)

normal.med=medians$Solid_Tissue_Normal
