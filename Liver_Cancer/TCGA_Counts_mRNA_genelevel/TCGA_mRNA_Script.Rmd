...
quartz()
par(mfrow=c(1,2))
barplot(colSums(counts(mergedds, normalized=FALSE)), col=conds_merged_colours2[coldata$Sample_Type], las=2,cex.names=0.5,main='Pre Normalised Counts')
legend("topleft",levels(conds_merged2),cex=0.5,fill=conds_merged_colours2[levels(conds_merged2)])

barplot(colSums(counts(mergedds, normalized=TRUE)), col=conds_merged_colours2[coldata$Sample_Type], las=2,cex.names=0.5,main='Post Normalised Counts')
legend("topleft",levels((conds_merged2)),cex=0.5,fill=conds_merged_colours2[levels((conds_merged2))])
```

#Alternative Normalisation? - TMM
```{r}
colnames(mergedcounts)[1:20]='GTEX_Normal'
colnames(mergedcounts)[21:32]=paste(pdata$Sample_Type)
...
y=DGEList(counts = mergedcounts, group = Type)
y=calcNormFactors(y)

dge=DGEList(counts = mergedcounts)
dge=calcNormFactors(dge, method = 'TMM')
design=model.matrix(~0+mergeddata$Sample_Type+mergeddata$Batch)
dge=estimateGLMCommonDisp(dge, design)
dge=estimateTagwiseDisp(dge, design)
fit = glmFit(dge, design)

quartz()
plotMDS(y, labels = colnames(mergedcounts), cex=0.4)
plotMDS(y, pch = 19, col= conds_merged_colours[conds_merged])
#Need to add text
```

#Alternative Normalisation? - SVA
```{r}
mod=model.matrix(~as.factor(mergeddata$Sample_Type), data = mergeddata)
mod0=model.matrix(~1, data = mergeddata)
svamerge=sva(mergedcounts, mod, mod0)
```
...
quartz()
plot(mergepca$x, main='PCA Variance Stabilised for Merged Data', pch=21, col='black', bg=conds_merged_colours[coldata$Sample_Type], cex=1)
text(mergepca$x, as.character(conds_merged2), pos=1, cex=0.4)
