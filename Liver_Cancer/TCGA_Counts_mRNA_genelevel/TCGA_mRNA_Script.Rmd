...
pdata=read.table('pdata_rnaseq_genelevel.txt', header=T)
rownames(pdata)=pdata$Sample
```
...
boxplot(log2(counts(mergedds, normalized=TRUE)+1), col=cond_colours[conds], las=2,cex.names=0.5,main='Post Normalised Counts')
```
...
barplot(lcpm, las=2, col=cond_colours[conds], main="Normalised Data",ylab="Log-cpm")
...
quartz()
plot(voompca$x, pch=c(21,24)[batch], bg=cond_colours[conds], cex=1, main='PCA Plot Edge R')
...
qqnorm(log2(dge$counts+1)[,31], main = 'Normal Q-Q Plot (TCGA Sample 1)')
qqnorm(log2(dge$counts+1)[,32], main = 'Normal Q-Q Plot (TCGA Sample 2)')
...
voompca=prcomp(t(v$E))
quartz()
plot(voompca$x, pch=c(21,24)[batch], bg=cond_colours[conds], cex=1, main = 'PCA Plot')
text(voompca$x,as.character(coldata$Batch),cex=0.3,pos=1)
text(voompca$x,as.character(colnames(v$E)),cex=0.3,pos = 3)
...
boxplot(log2(counts(ddssva, normalized=TRUE)+1), col=cond_colours[conds], las=2,cex.axis=0.5,main='Post Normalised Counts')
