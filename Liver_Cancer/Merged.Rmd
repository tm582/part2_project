...
```{r}
biomarkers = c('ENSG00000081051','ENSG00000147257', 'ENSG00000110492', 'ENSG00000135052', 'ENSG00000118785')
biomarkers.mat=mergevstcounts[biomarkers,]
biomarkers.mat=biomarkers.mat[,order(colnames(biomarkers.mat))]
...
cond_origin_cols=brewer.pal(length(unique(conds_origin)), 'Pastel2')

quartz()

heatmap.2(biomarkers.mat, trace='none', Colv = F, ColSideColors = cond_colours[conds[order(conds)]], labRow = gene_names[biomarkers,]$GeneName, main='Heatmap of VST counts for HCC Biomarkers')

quartz()
heatmap.2(biomarkers.mat, trace='none', Colv = F, ColSideColors = cond_origin_cols[conds_origin[order(conds_origin)]], labRow = gene_names[biomarkers,]$GeneName, main='Heatmap of VST counts for HCC Biomarkers')
...
mergepca.df = mergepca$x[, c(1,2,3)] #* -1 # if necessary flip coords for better visualisation
